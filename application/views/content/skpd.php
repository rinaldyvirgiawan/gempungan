    ...
    <div class="wrapper-half">
        ...
        <form action="<?php echo site_url('process/skpd/periksa/'.$dx) ?>" method="post" class="form-check form-global">
            ...
            <ul class="category-list list-nostyle">
                <?php
                $Qlist = $this->db->query("SELECT id, name FROM checklist WHERE id BETWEEN 18 AND 24 ORDER BY id ASC");
            ...
            <ul class="category-list list-nostyle">
                ...
                foreach($Qlist->result_object() as $list){
                    echo '<li>
                            <label class="checkbox">
                                <input type="checkbox" name="syarat[]" value="'.$list->id.'"';
                                foreach($Qedit2->result_object() as $edit2) if($edit2->checklist_id==$list->id) echo ' checked';
                                echo '>'.$list->name.'
                            </label>
                        <li>';
                }
