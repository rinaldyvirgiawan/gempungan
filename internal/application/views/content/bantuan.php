...
<div class="primary">
    <div class="project-detail-wrapper">
        ...
        <h1 class="page-title">Bantuan Keuangan</h1>
        ...
        <table class="table-global">
            ...
            <tbody>
                ...
                $Qlist = $this->db->query("SELECT a.surat_id, a.uraian, b.nphd_id, a.sppls_status, a.hibah_status, a.rekap_status
                                        ...
                                        WHERE a.jenis_bantuan='5' ORDER BY a.surat_id DESC LIMIT $position,$limit");                

                if($Qlist->num_rows){
                    $i = ($p*15)-14;

                    foreach($Qlist->result_object() as $list){
                        if($list->sppls_status==NULL && $_SESSION['internal']['role']==3 || $list->sppls_status==NULL && $_SESSION['internal']['role']==0) $sppls = '<a href="'.site_url('bantuan/sppls/'.$list->surat_id).'">PROSES</a>';
                        elseif($list->sppls_status==1 && $_SESSION['internal']['role']==4 || $list->sppls_status==1 && $_SESSION['internal']['role']==0) $sppls = '<a href="'.site_url('bantuan/sppls/'.$list->surat_id).'">PROSES</a>';
                        elseif($list->sppls_status==2 && $_SESSION['internal']['role']==5 || $list->sppls_status==2 && $_SESSION['internal']['role']==0) $sppls = '<a href="'.site_url('bantuan/sppls/'.$list->surat_id).'">PROSES</a>';
                        elseif($list->sppls_status==3 && $_SESSION['internal']['role']==6 || $list->sppls_status==3 && $_SESSION['internal']['role']==0) $sppls = '<a href="'.site_url('bantuan/sppls/'.$list->surat_id).'">PROSES</a>';
        ...
        <?php
        $Qpaging = $this->db->query("SELECT uraian FROM surat_masuk WHERE jenis_bantuan='5'");

        $num_page = ceil($Qpaging->num_rows / $limit);
        if($Qpaging->num_rows > $limit){
            $this->ifunction->paging($p, site_url('bantuan').'/'.$tp.'/', $num_page, $Qpaging->num_rows, 'href', false);
        }
        ?>             
    </div>
    ...
    <div class="project-detail-wrapper">
        ...
        <form action="<?php echo site_url('process/hibah/sppls/'.$p) ?>" method="post" class="form-check form-global">
            ...
            <div class="control-actions">
                ...
                <a href="<?php echo site_url('bantuan') ?>" class="btn-grey btn-plain btn" style="display:inline">Kembali</a>
            </div>
        </form>             
    </div>
    <!-- project-detail-wrapper -->
